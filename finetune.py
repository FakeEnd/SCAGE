...
class Trainer(object):
    ...
    def _get_net(self):
        ...
        if self.config['pretrain_model_path'] != 'None':
            state_dict = torch.load(os.path.join(self.config['pretrain_model_path'], 'model.pth'),
                                    map_location='cuda')
            model.model.load_model_state_dict(state_dict['model'])
            print("Loading pretrain model from", os.path.join(self.config['pretrain_model_path'], 'model.pth'))
        return model
...
if __name__ == '__main__':
    path = "config/config_finetune.yaml"
