...
from data_process.loader import MoleculeDataset, FinetuneDataset
...
class Trainer(object):
    ...
    def get_data_loaders(self):
        ...
        train_dataset = FinetuneDataset(root=self.config['root'] + self.config['task_name'],
                                        split_type='train',
                                        is_pretrain=self.config['is_pretrain'],
                                        target=self.config['target'],
                                        transform=TransformFn(self.config))

        val_dataset = FinetuneDataset(root=self.config['root'] + self.config['task_name'],
                                      split_type='valid',
                                      is_pretrain=self.config['is_pretrain'],
                                      target=self.config['target'],
                                      transform=TransformFn(self.config))

        test_dataset = FinetuneDataset(root=self.config['root'] + self.config['task_name'],
                                       split_type='test',
                                       is_pretrain=self.config['is_pretrain'],
                                       target=self.config['target'],
                                       transform=TransformFn(self.config))
